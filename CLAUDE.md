# 김천과외 (gimcheon-tutoring) — 작업 규칙

경상북도 김천시 지역 타겟 과외 사이트. 정적 사이트, 빌드 없음.
...
## 도메인 / 배포
- 도메인: **nadoclass.co.kr** (가비아, 2026-09-21 연결). `CNAME` 파일 있음 · 주소 `https://nadoclass.co.kr/`
...
- **검색엔진 소유확인 태그** (`index.html`, 2026-09-22 손으로 넣음): `naver-site-verification` = `51691e19706847a9a5c5fc10713cd83d0ba22a08` · `google-site-verification` = `72TUSduWAI-AuXDORtu2O4UOV66Y00q2CLItzZ8QHQM`. 생성기(`regions.js`)에는 아직 없어서 **재생성하면 사라진다** — 재생성 후 이 태그를 다시 넣거나, 생성기 head 부분에 사이트별 인증값을 추가할 것.
...
5. `& "$env:USERPROFILE\사이트관리\로그기록.ps1" -Repo gimcheon-tutoring -Status 완료 -What "글 N편"`
